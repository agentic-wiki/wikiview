...
**Not verified here: `-race`.** It needs cgo and this environment has no C compiler. `just test-race` runs it where one is available, and the tests were written to be race-free by construction for that reason.
